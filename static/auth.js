...
async function handleForgotPassword(e) {
    ...
    try {
        ...
        if (response.ok) {
            // Build direct link display if returned (bypassing email)
            let directLinkHtml = '';
            if (data.reset_link) {
                directLinkHtml = `
                    <div style="margin-top: 20px; padding: 12px; background: rgba(255, 159, 10, 0.08); border: 1px solid rgba(255, 159, 10, 0.3); border-radius: 8px; text-align: left;">
                        <span style="font-weight: bold; color: #ff9f0a; font-size: 13px; display: block; margin-bottom: 6px;"><i class="fa fa-info-circle"></i> Direct Reset Link (Bypassing Email):</span>
                        <a href="${data.reset_link}" style="color: #ff9f0a; word-break: break-all; font-size: 13px; font-weight: 500;">${data.reset_link}</a>
                    </div>
                `;
            }
            ...
            if (authForm) {
                authForm.innerHTML = `
                    <div style="text-align: center; padding: 20px 0;">
                        ...
                        ${directLinkHtml}
                        <a href="/login" class="auth-btn" style="display: inline-block; text-decoration: none; margin-top: 20px;">
...
function preserveQueryParamsInLinks() {
    ...
    document.querySelectorAll('a').forEach(link => {
        ...
        if (href && (href.startsWith('/') || href.startsWith('http'))) {
            try {
                ...
                console.error('Error preserving query params for link:', link, e);
            }
        }
    });
}
